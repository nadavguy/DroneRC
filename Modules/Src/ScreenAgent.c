...
uint8_t nextFrameToDraw[153600] = {0xFF};

char droneType[6] = "";

void screenInit(void)
{
	isPortrait = false;//(bool)ee.screenOreintation;
	DEV_Module_Init();
	LCD_2IN4_SetBackLight(ee.backLight * 2000);
	if (isPortrait)
	{
		LCD_2IN4_Init();
		LCD_2IN4_Clear(BLACK);
		Paint_NewImage(LCD_2IN4_HEIGHT,LCD_2IN4_WIDTH, 0, WHITE);
...
void setIconPositionOnScreen(void)
{
	...
	else
	{
		...
		SystemStatusTextX = HorizontalSystemStatusTextX;
		SystemStatusTextY = HorizontalSystemStatusTextY;

		SystemTextX = HorizontalSystemTextX;
		SystemTextY = HorizontalSystemTextY;

		WarningTextX = HorizontalWarningTextX;
		WarningTextY = HorizontalWarningTextY;

		LineStartX = HorizontalSafeAirLineStartX;
		LineEndX = HorizontalSafeAirLineEndX;
		LineY = HorizontalSafeAirLineY;

		AltitudeOrGPSX = HorizontalAltitudeOrGPSX;
		AltitudeOrGPSY = HorizontalAltitudeOrGPSY;


		SafeAirBatteryX = (numberOfDisplayedSafeAirIcons /2 - 1 ) * safeAirBarIconWidth + HorizontalDisplayCenterWidth;//VerticalSafeAirBatteryX;
		SafeAirBatteryY = HorizontalSafeAirBatteryIconY;
		TriggerModeX = (numberOfDisplayedSafeAirIcons /2 - 2 ) * safeAirBarIconWidth + HorizontalDisplayCenterWidth;
		TriggerModeY = HorizontalTriggerModeY;
		PlatfomTypeX = (numberOfDisplayedSafeAirIcons /2 - 3 ) * safeAirBarIconWidth + HorizontalDisplayCenterWidth;
		PlatfomTypeY = HorizontalPltfomTypeY;
		AutoPilotX = (numberOfDisplayedSafeAirIcons /2 - 4 ) * safeAirBarIconWidth + HorizontalDisplayCenterWidth;
		AutoPilotY = HorizontalAutoPilotY;

		ChargingModeImageX = HorizontalChargingModeX;
		ChargingModeImageY = HorizontalChargingModeX;
		ChargingModePercentTextX = HorizontalChargingModePercentX;
		ChargingModePercentTextY = HorizontalChargingModePercentY;
